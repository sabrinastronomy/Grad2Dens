...
class InferDens(SwitchMinimizer):
    ...
    """
    def __init__(self, seed, z, truth_field,  brightness_temperature_field):
        self.z = z
        self.seed = seed
    ...
    def infer_density_field(self):
        ...
        for iter_num_big in range(self.iter_num_max):
            ...
            if iter_num_big == 0:
                ...
                masked_field = masked_field.at[self.neutral_indices_shaped].set(-1)
            ...
            x = axes.imshow(self.best_field_reshaped, norm=matplotlib.colors.SymLogNorm(linthresh=0.01, vmin=-1))
            ...
            # fig.tight_layout()
            # fig.savefig(f"/Users/sabrinaberger/density_battaglia.png")
            # exit()
            batt_model_instance = Dens2bBatt(self.best_field_reshaped, delta_pos=1, set_z=self.z, flow=True)
            temp_model_brightness = batt_model_instance.temp_brightness
            ...
            if self.save_prior_likelihood_arr:
    ...
    def plot_pspecs(self):
        ...
        for i, k in enumerate([0, 1]):
            ...
            counts, pspec_pre, kvals = after_circular_spec_normal(best_field, self.num_bins, self.resolution, self.area)
            ...
            for j in range(cols):
                ...
                if i == 0 and j == 0:
                    if normalize:
                        ...
                        t = axes[i][j].text(20, self.side_length - 10, f"Truth Field", color="black", weight='bold')
                        t.set_bbox(dict(facecolor='white', alpha=1, edgecolor='white'))
                        axes[i][j].set_xticks([])
                        ...
                        cbar.ax.tick_params(labelsize=tick_font_size)
                        t = data_axes[i][j].text(20, self.side_length - 10, f"Truth Data", color="black", weight='bold')
                        t.set_bbox(dict(facecolor='white', alpha=1, edgecolor='white'))
                        ...
                        cbar.ax.tick_params(labelsize=tick_font_size)
                        t = residuals_axes[i][j].text(20, self.side_length - 10, f"Zero Residuals", color="black", weight='bold')
                        t.set_bbox(dict(facecolor='white', alpha=1, edgecolor='white'))
                ...
                if normalize:
                    ...
                    cbar = fig.colorbar(im, cax=cax, fraction=0.046, pad=0.04, orientation="horizontal")
                    cbar.ax.tick_params(labelsize=tick_font_size)
                    t = axes[i][j].text(20, self.side_length - 10, f"Iteration #{k} \nwith " + self.plot_title, color="black", weight='bold')
                    t.set_bbox(dict(facecolor='white', alpha=1, edgecolor='white'))
                    axes[i][j].set_xticks([])
                    axes[i][j].set_aspect('equal')
                    ...
                    cbar = residuals_fig.colorbar(im, cax=cax, fraction=0.046, pad=0.04, orientation="horizontal")
                    cbar.ax.tick_params(labelsize=tick_font_size)
                    t = residuals_axes[i][j].text(20, self.side_length - 10, f"(Current-Truth) Iteration #{k} \nwith " + self.plot_title, color="black", weight='bold')
                    t.set_bbox(dict(facecolor='white', alpha=1, edgecolor='white'))
                    residuals_axes[i][j].set_xticks([])
                    residuals_axes[i][j].set_aspect('equal')
                    ...
                    cbar = data_fig.colorbar(im, cax=cax, fraction=0.046, pad=0.04, orientation="horizontal")
                    cbar.ax.tick_params(labelsize=tick_font_size)
                    t = data_axes[i][j].text(20, self.side_length - 10, f"Iteration #{k} \nwith " + self.plot_title, color="black", weight='bold')
                    t.set_bbox(dict(facecolor='white', alpha=1, edgecolor='white'))
                    data_axes[i][j].set_xticks([])
                    data_axes[i][j].set_aspect('equal')
    ...
    def check_field(self, field, title, normalize=False, save=True, show=False, iteration_number=-1):
        ...
        fig.tight_layout()
        fig.savefig(f"density_battaglia.png")
...
class ConfigParam:
    def __init__(self, num_bins, nothing_off, plot_direc, side_length, physical_side_length,
                 ...
                 debug=False, use_truth_mm=False, save_prior_likelihood_arr=False):
...
if __name__ == "__main__":
    ...
    run_optimizer = True
    params = ConfigParam()
    samp = InferDens(run_optimizer=run_optimizer, seed=1010, z=10, truth_field=np.array([None]), brightness_temperature_field=np.array([None]),
                     num_bins=60, mask_ionized=False, iter_num_max=6, side_length=512, physical_side_length=256,
                     plot_direc="", weighted_prior=False, new_prior=True,
                     old_prior=False, nothing_off=False, verbose=False,
                     debug=False, use_truth_mm=False, noise_off=True)
    ...
    # data = np.load(plot_dir + "npy/data_field_7.npy")gParam:
    def __init__(self, num_bins, nothing_off, plot_direc, side_length, physical_side_length,
                 dimensions=2, iter_num_max=10, rest_num_max=3, noise_off=False,
                 save_posterior_values=False, run_optimizer=False, mse_plot_on=False,
                 weighted_prior=None, new_prior=False, old_prior=False, verbose=False,
